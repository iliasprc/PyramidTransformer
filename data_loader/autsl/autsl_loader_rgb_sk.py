...
class AUTSL_RGB_SK(Base_dataset):
    def __init__(self, config, mode, classes):
        ...
        Args:
            ...
            classes:
        """
        super(AUTSL_RGB_SK, self).__init__(config, mode, classes)
        # print(config)
        ...
        self.list_depth = sorted(glob.glob(os.path.join(config.cwd, f'train/*depth.mp4')))
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            vid_tensor,sk_tensor = self.load_video(os.path.join(self.data_dir, self.mode, vid_name),
            ...
            return vid_tensor,sk_tensor, target_label
        elif self.mode == 'val':
    ...
    def load_video(self, vid_path, sk_path, time_steps, dim=(224, 224), augmentation=False, padding=True,
        ...
        vid = tensor_imgs.permute(1, 0, 2, 3)
        #print(vid.shape,torch.from_numpy(skeleton_npy[:, selected, :]).float().shape)
        return (vid,torch.from_numpy(skeleton_npy[:, selected, :]).float())
