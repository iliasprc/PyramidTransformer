...
from data_loader.loader_utils import sampling_mode, pad_video, video_transforms, VideoRandomResizedCrop, \
    read_gsl_continuous
...
class GSL_SIv2(Base_dataset):
    def __init__(self, config, mode, classes):
        ...
        Args:
            ...
            classes:
        """
        super(GSL_SIv2, self).__init__(config, mode, classes)
        ...
        if (self.modality == 'RGB'):
            self.data_path = os.path.join(self.config.dataset.input_data, 'GSL_v3')
    ...
    def load_video_sequence(self, path, label,
        ...
        #assert len(boundaries) == len(images)
        h_flip = False
        ...
        if (self.augmentation):
            ...
            temporal_augmentation = int((np.random.randint(50, 100) / 100.0) * len(images))
        ...
        if len(boundaries) != len(images):
            boundaries = [0 for i in num_of_images]
        else:
            boundaries = [boundaries[i] for i in num_of_images]
       ...
       # assert len(boundaries) == len(images)
        ...
        indexes = []
        for word in labels:
            indexes.append(self.classes.index(word))
        y = torch.tensor(indexes, dtype=torch.int)
        y_boundaries = torch.tensor(boundaries, dtype=torch.int)
        return X1.permute(1, 0, 2, 3), y, y_boundaries
