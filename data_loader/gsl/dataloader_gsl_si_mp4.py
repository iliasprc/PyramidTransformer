...
class GSL_SI_mp4(Base_dataset):
    def __init__(self, config, mode, classes):
        ...
        Args:
            ...
            classes:
        """
        super(GSL_SI_mp4, self).__init__(config, mode, classes)
        ...
        if (self.modality == 'RGB'):
            self.data_path = os.path.join(self.config.dataset.input_data, 'SLR_Continuous')
    ...
    def load_video_sequence(self, path,
        ...
        img_sequence = []
        videogen = skvideo.io.vreader(os.path.join(self.data_path, path, 'vid.mp4'))
        videometadata = skvideo.io.ffprobe(os.path.join(self.data_path, path, 'vid.mp4'))
        print(os.path.join(self.data_path, path, 'vid.mp4'))
        print(videometadata)
        num_of_frames = int(videometadata['video']['@nb_frames'])
        num_of_images = list(range(num_of_frames))
        # print(images)
        ...
        if (self.augmentation):
            ...
            temporal_augmentation = int((np.random.randint(65, 100) / 100.0) * len(num_of_images))
            if (temporal_augmentation > 15):
                num_of_images = sorted(random.sample(num_of_images, k=temporal_augmentation))
            if (len(num_of_images) > self.seq_length):
                # random frame sampling
                num_of_images = sorted(random.sample(num_of_images, k=self.seq_length))

        else:
            # test uniform sampling
            if (len(num_of_images) > self.seq_length):
                num_of_images = sorted(sampling(num_of_images, self.seq_length))
        ...
        if (len(num_of_images) < 25):
            X1 = pad_video(X1, padding_size=25 - len(num_of_images), padding_type='images')
        # print(X1.shape)
        # import torchvision
        # import cv2
        # X2 = X1[1:]
