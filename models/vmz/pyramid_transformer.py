...
class PyramidTransformerResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, layers,
        ...
        """
        super(PyramidTransformerResNet, self).__init__()
        self.inplanes = 64
        ...
        # self.tpn1 = SpatialModulation(64 * block.expansion,64 * block.expansion, downsample_scale=64, k=7, s=3, d=5)
...
def ir_csn_152_transformer(pretraining="ig65m_32frms", pretrained=False, progress=False, num_classes=226, **kwargs):
    ...
    if pretrained:
        if pretraining in avail_pretrainings:
            ...
            pretrained = False
    use_pool1 = True
    model = PyramidTransformerResNet(block=Bottleneck, conv_makers=[Conv3DDepthwise] * 4, layers=[3, 8, 36, 3],
...
class TimesformerResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, layers,
        ...
        Args:
            ...
            zero_init_residual (bool, optional): Zero init bottleneck residual BN. Defaults to False.
        """
        super(TimesformerResNet, self).__init__()
        self.inplanes = 64
...
def ir_csn_152_timesformer(pretraining="ig65m_32frms", pretrained=False, progress=False, num_classes=226, **kwargs):
    ...
    if pretrained:
        if pretraining in avail_pretrainings:
            ...
            pretrained = False
    use_pool1 = True
    model = TimesformerResNet(block=Bottleneck, conv_makers=[Conv3DDepthwise] * 4, layers=[3, 8, 36, 3],
                              stem=BasicStem_Pool if use_pool1 else BasicStem, num_classes=num_classes, **kwargs)
