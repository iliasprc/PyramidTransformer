...
from data_loader.dataset import SLDetection_DataModule
from models import CSLR_video_encoder,SLD_model
...
def main():
    ...
    if len(myargs) > 0:
        ...
        config_file = 'config/SLDetection/trainer_config.yml'
    ...
    datamodule = SLDetection_DataModule(config)
    datamodule.setup()
    ...
    model = SLD_model(config, 1)
    if (config.load):
        model.fc = torch.nn.Sequential(torch.nn.Linear(1024, 311))
        pth_file, _ = load_checkpoint(config.pretrained_cpkt, model, strict=True, load_seperate_layers=False)
        model.fc = torch.nn.Linear(1024,1)
    # # if config.load:
