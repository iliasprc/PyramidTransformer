...
class TrainerRGBDSK(BaseTrainer):
    """
    Trainer class
    """
    ...
    def __init__(self, config, model, optimizer, data_loader, writer, checkpoint_dir, logger,
        ...
        super(TrainerRGBDSK, self).__init__(config, data_loader, writer, checkpoint_dir, logger,
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_data_loader):
            rgb_tensor, depth_tensor, sk, y = data
            rgb_tensor = rgb_tensor.to(self.device)
            depth_tensor = depth_tensor.to(self.device)
            sk = sk.to(self.device)
            y = y.to(self.device)

            output, loss, target = self.model((rgb_tensor, depth_tensor,sk, y))
