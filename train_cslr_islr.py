...
from trainer_cslr.trainer_CSLR_ISLR import Trainer_CSLR_method
...
def main():
    ...
    training_generator, val_generator, test_generator, classes, id2w, gloss_to_index = islr_datasets(config)
    ...
    if (config.load):
        ...
        model.fc_gloss = torch.nn.Linear(1024, 311)
        model.fc_sentence = torch.nn.Linear(1024, len(classes))
    ...
    trainer = Trainer_CSLR_method(config, model=model, optimizer=optimizer,
